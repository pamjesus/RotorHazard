...
bool RssiNode::checkForCrossing_ph(const ExtremumType currentType, const uint8_t threshold)
{
    ...
    if (state.crossing && prevType == PEAK && currentType == NADIR) {
        Extremum currentNadir[] = {history.nadir};
        ArrayList<Extremum,1> currentList(currentNadir);
        CompoundList<Extremum,HISTORY_SIZE,1> pns(sendBuffer, currentList);
        uint8_t numCCs = calculateNadirPersistentHomology(pns, sendBuffer.typeAt(0), ccs);

        // find lifetime of last value when a nadir
        uint8_t lastLife = 0;
        const uint8_t lastIdx = pns.size() - 1;
        for (int i=0; i<numCCs; i++) {
            ConnectedComponent& cc = ccs[i];
            if (cc.birth == lastIdx) {
                lastLife = pns[cc.death].rssi - pns[cc.birth].rssi;
                break;
            }
        }
        ...
        Extremum currentPeak[] = {history.peak};
        ArrayList<Extremum,1> currentList(currentPeak);
        CompoundList<Extremum,HISTORY_SIZE,1> pns(sendBuffer, currentList);
        uint8_t numCCs = calculatePeakPersistentHomology(pns, sendBuffer.typeAt(0), ccs);
        ...
        for (int i=0; i<numCCs; i++) {
            ConnectedComponent& cc = ccs[i];
            if (cc.birth == lastIdx) {
                lastLife = pns[cc.birth].rssi - pns[cc.death].rssi;
                break;
            }
        }
